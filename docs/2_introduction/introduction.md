...
- Adaptation
  ...
  - D'adaptation ensuite. L'adaptation est une modification souhaitée d'un système pour faire face à une perturbation. Dans notre cas, le système est la bande littorale en Martinique, et la perturbation est le changement climatique. Plus précisément, à travers différents canaux, celui-ci induit plusieurs perturbations, notamment l'augmentation de la puissance des vagues, le recul du trait de côte et une surcote marine.
    - Première définition
      - un processus qui permet de maintenir le fonctionnement d'un système face au changement climatique
        - [[6a030d71-f461-440c-8cb0-3a5946b19227]]
        - c'est donc à la fois un processus, c'est à dire quelque chose d'actif, avec des mesures, des décisions, etc; et quelque choses pour limiter les impacts, pour se protéger
        - Donc c'est assez simple : l'adaptation c'est une série de mesure qu'il faut prendre pour limiter l'impact. Donc avec cette approche là, on se dit qu'il suffit de voir les mesures qui existent, mesurer lesquelles sont les plus efficaces / efficientes, puis les mettre en œuvre.
    - Enjeux autour de la notion : pourquoi ça nous intéresse ?
      - Pourtant c'est un peu plus compliqué que ça, car le concept est largement utilisé mais peu défini
        - déjà, c'est un concept profondément polysémique => cf. Reghezza - Zitt : ça ne veut pas dire la même chose pour tout le monde
          - ainsi Reghezza-Zitt montre que c'est un terme extremement employé, qui est utilisé parfois de manière contradictoire
        - il y a notamment un double sens ancien dans le terme d'adaptation : à la fois un processus et une finalité
          - “’étymologie du terme « adaptation » fait référence à la fois à un processus (action) et à une finalité (résultat).”
    - Comment on va la mobiliser ?
